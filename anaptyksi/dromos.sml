	...
	let
		fun check length _ distance _ telos [] = if (length - telos) > distance then 0 else 1
		  | check length stop distance begin telos ((beg, endi, day)::xs) =
				if day > stop then check length stop distance begin telos xs
				else
					let
		...
		fun binary ans n l x low high xs =
			let
				val some =
					...
					else if (check l ((low + high) div 2) x 0 0 xs) = 1
						then binary ((low + high) div 2) n l x low (((low + high) div 2) - 1) xs
						else binary ans n l x (((low + high) div 2) + 1) high xs
			in
				some
			end
